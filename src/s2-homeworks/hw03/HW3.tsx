...
export const pureAddUserCallback = (
    ...
    const user: UserType = {
        _id: v1(), // Генерируем уникальный ID с помощью uuid v1
        name: name,
    };
    setUsers([...users, user]);
};
const HW3 = () => {
    const [users, setUsers] = useState<Array<UserType>>([]); // Указываем правильный тип для users

    const addUserCallback = (name: string) => { // Указываем правильный тип для name
